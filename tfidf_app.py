...
@st.cache_resource
def load_artifacts():
    """Loads the pre-trained TF-IDF model and vectorizer for sentiment analysis."""
    try:
        with open('TfIdf.pickle', 'rb') as f:
            vectorizer = pickle.load(f)
        # Assuming the TF-IDF model is also saved as model.pickle in its respective folder
        with open('tfidf_model.pickle', 'rb') as f:
            model = pickle.load(f)
        return vectorizer, model
    except FileNotFoundError:
        st.error("Model/vectorizer files not found. Ensure 'TfIdf.pickle' and 'model.pickle' are present.")
        return None, None
...
def main():
    st.set_page_config(page_title="Kindle Sentiment Analyzer (TF-IDF)", page_icon="📖", layout="centered")
    st.title("📖 Kindle Review Sentiment Analyzer (TF-IDF)")
    st.markdown("Enter a Kindle review to classify its sentiment as **Positive** or **Negative** using a TF-IDF model.")
    
    message_input = st.text_area("Enter review text here:", "This book was a fantastic read, I couldn't put it down!", height=150)
        ...
        if message_input and vectorizer and model:
            with st.spinner('Analyzing...'):
                ...
                if prediction == 1: # 1 for Positive sentiment
                    ...
                    st.error("This review seems Negative.")
